...
namespace TimeServer19
{
	public class Protocol
	{
		public static string title = "Time Server 19(Sample)";
	...
	class Program
	{
		...
		void OnAcceptAsync(object _obj, SocketAsyncEventArgs _acceptArgs)
		{
			...
			if (Protocol.DEBUG)
				Console.WriteLine("[{0}]OnAcceptAsync (callback) New Client Connect"
				   ...
				   identity, acceptSocket.Connected, _acceptArgs.BytesTransferred, _acceptArgs.LastOperation,
				   _acceptArgs.SocketError);

			Console.WriteLine($"[{Thread.CurrentThread.ManagedThreadId}]스레드id");
			//---------------------------------------
		...
		void OnSendAsync(object _obj, SocketAsyncEventArgs _sendArgs)
		{
			...
			{
				//메세지 보냈으니까 보내것 큐에서 빼고 > 더있는지 검사후에 다음것 등록...
		...
		void Disconnect(string _msg, CUserToken _token)
		{
			...
			Console.WriteLine(" [{0}] release {3} free:{1} use:{2}", _token.identityID, list_Pool.Count, list_Users.Count, _msg);
			_token.ClearData();
		}
	}
	...
	public class CUserToken
	{
		...
		public void SendMessage(byte[] _data, int _debugWorkNum = -1)
		{
			if (Protocol.DEBUG_PACKET)
				Console.WriteLine("[{0}]SendMessage >> sendQueue.Count:{1}",
				   identityID + "/" + _debugWorkNum, sendQueue.Count);
		...
		void SendMessage_Sending(int _debugWorkNum)
		{
			if (Protocol.DEBUG_PACKET)
				Console.WriteLine("[{0}]SendMessage_Sending >> ", identityID + "/" + _debugWorkNum);
