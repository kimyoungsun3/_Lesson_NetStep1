...
namespace JWT4_Sell_ask
{
	public class Program
	{
		...
		void RunUpbit()
		{
			...
			decimal COIN_PRICE_ASK = 99.2m + 1.0m * 0;
			int COIN_HOGA_ASK = +1;
			int ORDER_ASK_LOOP = 100;
			...
			for (int i = 0, j = 0; i < ORDER_ASK_LOOP * COIN_HOGA_ASK; i++)
			{
				if (i % Math.Abs(COIN_HOGA_ASK) == 0)
				{
					orderbook.SetASK(askCount, askPrice);
					listASK.Add(orderbook);
					Console.WriteLine(j++ + " > " + askPrice + ":" + askCount);
				}
			...
			////----------------------------------------------------
			//string coinCode = "KRW-CPT";
			//decimal COIN_PRICE_BID = 9.51m - .3m * 0;
			//int COIN_HOGA_BID = -1;
